...
// <Route> : 화면에 그려지는 페이지 객체, Route 컴포넌트에 자식 컴포넌트로 Route 컴포넌트를 사용 시 부모 Route 컴포넌트의 주소를 기본 주소로 사용함
  // path : 웹 브라우저에 표시되는 url 주소를 설정, 절대 경로/상대 경로 다 사용 가능, * 을 사용 시 모든 페이지를 뜻함(path url을 정확하기 입력해야 접속이 가능, 주로 error 페이지 설정 시 사용함)
...
// useSearchParams() 훅을 사용하 데이터를 가져오면 파라미터 데이터만 따로 가져올 수 있음
// 사용법 ;
