...
class SetStateClass extends React.Component {
  ...
  StateChange = (flag) => {
    if (flag == 'direct') {
      ...
      // 리액트는 Virtual DOM을 사용하기 때문에 state의 내용을 직접 수정하면 수정된 내용이 VirtualDOM에만 적용되고 DOM에는 적용되지 않음
  ...
  render() {
    return (
      <div>
        {/* jsx문법에서 문자열만 {} 없이 사용 가능, {} 사용 시 자바스크립트 코드임 */}
        {/* jsx문법에서 css 속성 사용 시 '-' 기호를 생략하고 카멜명명법 사용 */}
        {/* jsx문법에서 클릭 이벤트가 onclick -> onClick로 변경 */}
